...
import { HiCheck, HiMiniCheck } from "react-icons/hi2";
...
function StepperIndicator({
  ...
  return (
    <span
      ...
      className={cn(
        "bg-neutral-200 text-muted-foreground data-[state=active]:bg-base-heading data-[state=completed]:bg-base-heading data-[state=active]:text-white data-[state=completed]:text-white relative flex size-6 shrink-0 items-center justify-center rounded-full text-xs font-medium",
        className,
      )}
      data-state={state}
      {...props}
    >
      {asChild ? (
        ...
        <>
          ...
          {
            (step == 1 && !shippingCookie) ? (
              <FiX
                strokeWidth={3}
                className="absolute ml-[1px] scale-0 opacity-0 transition-all group-data-[state=completed]/step:scale-100 group-data-[state=completed]/step:opacity-100"
                size={15}
                aria-hidden="true"
              />
            ) : (
              <HiMiniCheck
                strokeWidth={1}
                className="absolute ml-[0.5px] scale-0 opacity-0 transition-all group-data-[state=completed]/step:scale-100 group-data-[state=completed]/step:opacity-100"
                size={15}
                aria-hidden="true"
              />
            )
